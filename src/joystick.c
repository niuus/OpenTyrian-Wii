...
void poll_joystick( int j )
{
	...
	int d;
	for (d = 0; d < COUNTOF(joystick[j].direction); d++)
	{
	...
	for (d = 0; d < COUNTOF(joystick[j].action); d++)
	{
...
void push_joysticks_as_keyboard( void )
{
	...
	for (j = 0; j < joysticks; j++)
	{
		...
		for (d = 0; d < COUNTOF(joystick[j].direction_pressed); d++)
		{
...
void reset_joystick_assignments( int j )
{
	...
	int a,i;
	for (a = 0; a < COUNTOF(joystick[j].assignment); a++)
	{
		...
		for (i = 1; i < COUNTOF(joystick[j].assignment[a]); i++)
		{
...
bool load_joystick_assignments( int j )
{
	...
	for (a = 0; a < COUNTOF(joystick[j].assignment); a++)
	{
		for (i = 0; i < COUNTOF(*joystick[j].assignment); i++)
		{
